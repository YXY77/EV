...
data6=pd.read_csv('./data/income_2018.csv')                       #Income
data7=pd.read_csv('./data/Job_population_per_county_2018.csv')    #Number of Employment
data8=pd.read_csv('./data/Population_county_2018.csv')            #Population

#Data Preprocessing for Data collected in 2018
#(1)Creating new DataFrame for the dependent vaiable, i.e. number of EV per County.
df_EV= {'County' : data1.loc[data1['Model Year']==2018]['County'].value_counts().index,
            'EV_number': data1.loc[data1['Model Year']==2018]['County'].value_counts()
           }
df_EV=pd.DataFrame(df_EV)


#(2)Creating new DataFrame for the pollution-realted vaiables
pollution=data3[data3['Year']==2018].groupby('County').mean().to_numpy()[:,2:7]
data3_county=data3[data3['Year']==2018].groupby('County').mean().index.to_numpy()
